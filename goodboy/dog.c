/*
 * dog.c
 * game where dog avoids touching fire hydrants
 */
...
void setup_background() {
    ...
    *bg1_control = 0 |    /* priority, 0 is highest, 3 is lowest */
    ...
    memcpy16_dma((unsigned short*) screen_block(16), (unsigned short*) dogmap, dogmap_width * dogmap_height);
}
...
void setup_sprite_image() {
    /* load the palette from the image into palette memory*/
    memcpy16_dma((unsigned short*) sprite_palette, (unsigned short*) dog_palette, PALETTE_SIZE);

// memcpy16_dma((unsigned short*) sprite_palette, (unsigned short*) fire_palette, PALETTE_SIZE);


    /* load the image into sprite image memory */
    memcpy16_dma((unsigned short*) sprite_image_memory, (unsigned short*) dog_data, (dog_width * dog_height) / 2);

//memcpy16_dma((unsigned short*) sprite_image_memory, (unsigned short*) fire_data, (fire_width * fire_height) / 2);


}

/* a struct for the dog's logic and behavior */
struct Dog {
    ...
    /* the dog's y velocity in 1/256 pixels/second */
    int yvel;

    /* the dog's y acceleration in 1/256 pixels/second^2 */
    int gravity; 
    ...
    /* whether the dog is moving right now or not */
    int move;

    /* the number of pixels away from the edge of the screen the dog stays */
    int border;

    /* if the dog is currently falling */
    int falling;
};


/* struct for fire hydrant and the stuff it does*/ 
...
void dog_init(struct Dog* dog) {
    ...
    dog->counter = 0;
    dog->falling = 0;
    dog->animation_delay = 8;
    dog->sprite = sprite_init(dog->x >> 8, dog->y >> 8, SIZE_8_8, 0, 0, dog->frame, 0);
}
...
/* move the dog left or right returns if it is at edge of the screen */
int dog_left(struct Dog* dog) {
    /* face left */
    sprite_set_horizontal_flip(dog->sprite, 1);
    dog->move = 1;

    /* if we are at the left end, just scroll the screen */
    if ((dog->x >> 8) < dog->border) {
        return 1;
    } else {
        /* else move left */
        dog->x -= 256;
...
int dog_right(struct Dog* dog) {
    /* face right */
    sprite_set_horizontal_flip(dog->sprite, 0);
    dog->move = 1;

    /* if we are at the right end, just scroll the screen */
    if ((dog->x >> 8) > (SCREEN_WIDTH - 16 - dog->border)) {
        return 1;
    } else {
        /* else move right */
        dog->x += 256;
...
/* stop the dog from walking left/right */
void dog_stop(struct Dog* dog) {
    dog->move = 0;
    dog->frame = 0;
    dog->counter = 7;
    sprite_set_offset(dog->sprite, dog->frame);
}

/* start the dog jumping, unless already fgalling */
void dog_jump(struct Dog* dog) {
    if (!dog->falling) {
        dog->yvel = -1350;
        dog->falling = 1;
    }
...
/* update the dog */
void dog_update(struct Dog* dog, int xscroll) {
    /* update y position and speed if falling */
    if (dog->falling) {
        dog->y += dog->yvel;
        dog->yvel += dog->gravity;
    }

    /* check which tile the dog's feet are over */
    unsigned short tile = tile_lookup((dog->x >> 8) + 8, (dog->y >> 8) + 32, xscroll,
            0, dogmap, dogmap_width, dogmap_height);

    /* if it's block tile
     * these numbers refer to the tile indices of the blocks the dog can walk on */
    if ((tile >= 1 && tile <= 6) || 
        (tile >= 12 && tile <= 17)) {
        /* stop the fall! */
        dog->falling = 0;
    ...
    if (dog->move) {
        dog->counter++;
        if (dog->counter >= dog->animation_delay) {
            dog->frame = dog->frame + 16;
            if (dog->frame > 16) {
                dog->frame = 0;
            }
            sprite_set_offset(dog->sprite, dog->frame);
    ...
    sprite_position(dog->sprite, dog->x >> 8, dog->y >> 8);
}
...
/* call assembly function */
    ...
    while (1) {
        /* update the dog */
        dog_update(&dog, xscroll);

        /* now the arrow keys move the dog */
